...
const whitelist = (v: string) => v.replace(/[^0-9A-Za-zÀ-ÿ\s.,;:()\-\/+%]/g, '');
...
const jitter = (ms: number) => ms + Math.floor(Math.random() * Math.min(250, ms / 2));
const safeFetch = async (
  ...
  while (attempt <= retries) {
    ...
    try {
      ...
      const signalToUse = linkedController ? linkedController.signal : ac.signal;
    ...
    attempt += 1;
    if (attempt <= retries)
      await new Promise((r) => setTimeout(r, jitter(200 * Math.pow(2, attempt))));
  }
  throw lastErr instanceof Error ? lastErr : new Error('Falha de rede');
};
const persistSignedPrescription = async (p: SignedPrescription, controller: AbortController) => {
...
const useSecurity = () => {
  ...
  const signPrescription = useCallback(
    async (data: Omit<SignedPrescription, 'id' | 'signature' | 'signedAt'>) => {
      ...
      const digest = await crypto.subtle.digest('SHA-256', enc.encode(`${payload}.${csrf}.${tok}`));
...
const baseFont = 'system-ui,-apple-system,Segoe UI,Roboto,Ubuntu,Cantarell,Noto Sans,sans-serif';
  ...
  return (
    <div style={s} aria-live="polite">
      <svg width="18" height="18" viewBox="0 0 24 24" aria-hidden="true" focusable="false">
        <path d="M12 2l7 3v6c0 5.25-3.5 9.92-7 11-3.5-1.08-7-5.75-7-11V5l7-3z" fill="#0ea5b7" />
      </svg>
...
const DigitalPrescription: React.FC = () => {
  ...
  const preview = useMemo(() => (dInstructions ? dInstructions.slice(0, 80) : ''), [dInstructions]);
  ...
  const validateLocal = useCallback((p: PrescriptionInput) => {
    ...
    if (typeof p.dosage !== 'string' || p.dosage.length < 1) e.dosage = 'Informe a dose';
    if (typeof p.frequency !== 'string' || p.frequency.length < 1)
      e.frequency = 'Informe a frequência';
    if (typeof p.duration !== 'string' || p.duration.length < 1) e.duration = 'Informe a duração';
    if (typeof p.instructions !== 'string' || p.instructions.length > 800)
      e.instructions = 'Instruções com máximo de 800 caracteres';
    return e;
  }, []);

  const focusFirstError = useCallback((e: Partial<Record<FieldKey, string>>) => {
    const order: FieldKey[] = ['medication', 'dosage', 'frequency', 'duration', 'instructions'];
    ...
    <Container>
      ...
      >
        <div style={row}>
          ...
          />
          {errors.medication ? (
            <span id="err-medication" role="alert" style={{ color: '#b91c1c', fontSize: 12 }}>
              {errors.medication}
            </span>
          ) : null}
        </div>
        ...
        <div style={grid2}>
          <div style={row}>
            ...
            />
            {errors.dosage ? (
              <span id="err-dosage" role="alert" style={{ color: '#b91c1c', fontSize: 12 }}>
                {errors.dosage}
              </span>
            ) : null}
          </div>
          ...
          <div style={row}>
            ...
            />
            {errors.frequency ? (
              <span id="err-frequency" role="alert" style={{ color: '#b91c1c', fontSize: 12 }}>
                {errors.frequency}
              </span>
            ) : null}
          </div>
        ...
        <div style={grid2}>
          <div style={row}>
            ...
            />
            {errors.duration ? (
              <span id="err-duration" role="alert" style={{ color: '#b91c1c', fontSize: 12 }}>
                {errors.duration}
              </span>
            ) : null}
          </div>
          ...
          <div style={row}>
            ...
            <textarea
              ...
              aria-describedby={errors.instructions ? 'err-instructions' : undefined}
              value={prescription.instructions}
              onChange={(e) => setField('instructions', e.currentTarget.value)}
            />
            {errors.instructions ? (
              <span id="err-instructions" role="alert" style={{ color: '#b91c1c', fontSize: 12 }}>
                {errors.instructions}
              </span>
            ) : null}
          </div>
      ...
      >
        ...
        {preview ? (
          <div style={{ fontSize: 12, color: '#475569' }}>Prévia das instruções: “{preview}”</div>
        ) : null}
      </div>
