...
app.get('/api/pacientes', async (req, res) => {
  ...
  const filtered = q
    ? items.filter(
        (p) => (p.nome || '').toLowerCase().includes(q) || (p.doc || '').toLowerCase().includes(q),
      )
    : items;
...
app.get('/api/pacientes/quick', async (req, res) => {
  ...
  let list = t
    ? items.filter(
        (p) => (p.nome || '').toLowerCase().includes(t) || (p.doc || '').toLowerCase().includes(t),
      )
    : items;
  list = list.sort((a, b) => (a.nome || '').localeCompare(b.nome || '')).slice(0, 12);
  res.json({
    ok: true,
    items: list.map((p) => ({ id: p.id, nome: p.nome, nascimento: p.nascimento, doc: p.doc })),
  });
});
