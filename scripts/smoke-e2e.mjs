...
async function listPatients(query) {
  const res = await fetch(`${BASE}/api/v1/patients?query=${encodeURIComponent(query)}`);
  if (!res.ok) throw new Error(`List failed: ${res.status}`);
  return res.json();
}

async function getEvents(patientId) {
  const res = await fetch(`${BASE}/api/v1/patients/${encodeURIComponent(patientId)}/events`);
  if (!res.ok) throw new Error(`Events failed: ${res.status}`);
  return res.json();
}

async function removePatient(patientId) {
  const res = await fetch(`${BASE}/api/v1/patients/${encodeURIComponent(patientId)}`, { method: 'DELETE' });
...
async function main() {
  ...
  try {
    ...
    console.log('Events for patient:', Array.isArray(events.items) ? events.items.length : 0);
  } finally {
    if (created?.id) {
