...
async function appendAuditLog(action,meta){
  ...
  payload.hash=createHash("sha256").update(`${prev}:${JSON.stringify({action,meta})}`).digest("hex");
...
setInterval(()=>{cleanupIdempotency().catch(()=>undefined);},60*60*1000);
...
app.post("/api/pacientes", async (req,res)=>{
  ...
  if(!(b.nome&&b.nascimento)){
    if(key) await persistIdempotency(key,"ERROR",{ok:false,error:"nome/nascimento"});
  ...
  await appendTimelineEvent({id:`evt_${id}`,patientId:id,type:"PATIENT_CREATE",createdAt:novo.criadoEm,payload:{}});
...
app.post("/api/pacientes/:id/evolucoes", async (req,res)=>{
  ...
  if(!texto){
    if(key) await persistIdempotency(key,"ERROR",{ok:false,error:"texto vazio"});
  ...
  await appendTimelineEvent({id:`evt_evo_${evo.id}`,patientId:req.params.id,type:"NOTE_CREATE",createdAt:evo.data,payload:{tipo:evo.tipo}});
  await appendAuditLog("NOTE_CREATE",{patientId:req.params.id,noteId:evo.id});
  if(key) await persistIdempotency(key,"OK",payload);
  res.json(payload);
});
...
app.post("/api/prescricoes/print", async (req,res)=>{
  ...
  if(!pacienteId){
    if(key) await persistIdempotency(key,"ERROR",{ok:false,error:"pacienteId obrigatório"});
  ...
  if(items.length===0){
    if(key) await persistIdempotency(key,"ERROR",{ok:false,error:"Informe ao menos um item"});
  ...
  if(!paciente){
    if(key) await persistIdempotency(key,"ERROR",{ok:false,error:"Paciente não encontrado"});
  ...
  await appendAuditLog("PRESCRIPTION_PRINT",{patientId:pacienteId,prescriptionId:presc.id,numero:presc.numero});
...
app.get("/api/pacientes/:id/export", async (req,res)=>{
  ...
  const prescricoes=await readJson(path.join(dataDir,"prescricoes.json"),[]);
  const events=await readJson(timelineFile,[]);
  const payload={
    paciente:{
      id:paciente.id,
  ...
  await appendAuditLog("EXPORT_PATIENT_JSON",{patientId,exportedAt:new Date().toISOString()});
