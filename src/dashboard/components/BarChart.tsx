  ...
  useEffect(() => {
    ...
    async function initChart() {
      ...
      if (!Array.isArray(labels) || !Array.isArray(values)) {
        ...
        console.error(
          'BarChart: "labels" and "values" must be arrays. Provide real data via props/context.',
        );
        return;
      }
      if (labels.length !== values.length) {
        ...
        console.error(
          'BarChart: "labels" length must match "values" length. Pass aligned arrays via API/props.',
        );
        return;
      }
      ...
      try {
        ...
        const ChartModule = await import("chart.js/auto").catch(
          () => undefined,
        );
        const Chart =
          ChartModule && (ChartModule as any).default
            ...
            datasets: [
              {
                ...
                data: values,
                backgroundColor:
                  colors?.background ?? "rgba(54, 162, 235, 0.5)",
        ...
        console.error(
          "BarChart: failed to initialize chart library. Provide chart.js or initialize alternate library.",
          e,
        );
      }
    }
